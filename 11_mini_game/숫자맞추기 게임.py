...
while True: #난이도 선택 유효성 검사
    difficulty = input("난이도를 선택해주세요. (쉬움, 보통, 어려움) :")
    if difficulty in difficulty_list:
        break
    else:
        print("쉬움 보통 어려움 중에 선택 해주세요.")
        continue
...
while True:
    print(f"현재 시도 횟수 : {try_count} / {max_try}")
    if try_count >= max_try:
        print(f"시도 횟수를 초과하였습니다.\n 정답: {correct_answer}")
        break
    ...
    if guess == correct_answer:
        print(f"정답입니다 {try_count}번 만에 맞추었습니다")
        break
    elif guess < correct_answer: #정답이 입력한 숫자보다 큰 경우
        print("up")
    else: #정답이 입력한 숫자보다 작은 경우
        print("down")
